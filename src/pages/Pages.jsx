...
const Pages = () => {
  ...
  useEffect(() => {
    ...
    // const searcDet = fix_data.find((item) => item.id_posts == id)
    const searcDet = fix_data.filter((item) => item.id_posts == id)
    // console.log(searcDet);
    setDetail(searcDet)
  ...
  return (
    <div className="mx-4 lg:mx-56 my-5">
      ...
      <div className="flex gap-x-8 rounded-lg mb-5">
        <img
          alt="Home"
          src="https://images.unsplash.com/photo-1613545325278-f24b0cae1224?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1770&q=80"
          className="h-56 w-2/3 rounded-md object-cover hover:shadow-xl hover:shadow-indigo-100 bg-slate-200"
        />
        ...
        {dataDetail.length > 0 &&
          dataDetail?.map((dd) => {
            return (
                <article className="overflow-hidden rounded-lg shadow transition hover:shadow-lg my-4 w-2/3" key={dd.id_posts}>

                  <div className="bg-slate-50 p-4 sm:p-6">
                    <time className="block text-xs text-gray-500"> {humanDiffTime(new Date(dd.updated_at), new Date())} </time>
                    <h1 className="mt-0.5 text-5xl text-gray-900">{dd.post_title}</h1>
                    <p className="mt-2 text-sm/relaxed text-gray-500 prose" dangerouslySetInnerHTML={{ __html: `${dd.post_content}` }}>
                    </p>
                  </div>
                </article>
            )
